	...
	<form action="${path}/board/update" method="post" enctype="multipart/form-data" >
		...
		<table id="tbl-board">
			...
			<tr>
				...
				<td>
					<select name="type">
					    <option value="B1" ${board.type == 'B1' ? 'selected':''}>일반</option>
					    <option value="B2" ${board.type == 'B2' ? 'selected':''}>QnA</option>
					    <c:if test="${board.type == 'B3'}">
						    <option value="B3" selected>공지사항</option>
						</c:if>		
					    <c:if test="${board.type != 'B3'}">
						    <option value="B3">공지</option>
						</c:if>		
